...
function AddGameForm () {
    ...
    const [buttonText, setButtonText] = useState("Submit New Game");

    // function changeButtonText () {
    //     if (clicked) {

    //         setButtonText("Hide Submission Form")
    //     }
    // }

    return (
        <div>
            <button onClick={() => {setButtonText("Hide Submission Form")}} onClick={() => {setClicked(!clicked)}}>{buttonText}</button>
            {clicked && (
