...
function AddGameForm ({setGameData}) {
    ...
    function postRequest(newGame) {
        axios.post('http://localhost:8080/allGames', newGame)
    ...
    function onSubmit(event) {
        ...
        setSubmitted(true);
        const newGame = {name: name, img: img, cross_platform: crossPlatform, price: price, genres: genres}
        postRequest(newGame);
        setGameData((oldInfo)=> {
            console.log(oldInfo)
