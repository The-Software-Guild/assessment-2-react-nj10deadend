import Button from '@mui/material/Button';
import DeleteIcon from '@mui/icons-material/Delete';
import Typography from '@mui/material/Typography';
import Box from '@mui/material/Box';

function GameCards ({eachGame}) {

    return (
        <Box className="game-cards" onClick={(event) => console.log(event.target)}>
            <Typography variant="h2" component="box" gutterBottom>{eachGame.name}</Typography>
            <hr className="divider"></hr>
            <img className="imgs"src={eachGame.img} alt={eachGame.name} />
            <Typography variant="p" component="box" gutterBottom>Cross-Platform: {eachGame.cross_platform.toString()}</Typography>
            <Typography variant="p" component="box" gutterBottom>Price: $ {eachGame.price}</Typography>
            <Typography variant="p" component="box" gutterBottom>
                <p>Genres: </p>
                {eachGame.genres.map(eachGenre => {
                    return <Typography variant="li" component="box" key={eachGenre} gutterBottom>{eachGenre}</Typography>
                })}

            </Typography>
            <Typography variant="p" component="box" gutterBottom>ID: {eachGame._id}</Typography>
            {/* <Button onClick={(event) => console.log(event.target)} type="submit" variant="outlined" color="error" startIcon={<DeleteIcon />}>Delete Game</Button> */}



        </Box>
    )
}

export default GameCards;